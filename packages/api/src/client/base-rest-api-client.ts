...
export interface BaseRestApiClientConfig {
  baseURL?: string;
  onUnauthorized?: () => void;
  redirectPath?: string;
}
...
export default class BaseRestApiClient {
  ...
  constructor(config?: BaseRestApiClientConfig) {
    this.baseURL = config?.baseURL || process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
    this.redirectPath = config?.redirectPath || '/login';
    if (config?.onUnauthorized) {
      BaseRestApiClient.onUnauthorized = config.onUnauthorized;
    }
  }
