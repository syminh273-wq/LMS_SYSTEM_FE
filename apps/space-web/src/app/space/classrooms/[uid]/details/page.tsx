...
function ExamGradeTableModal({
  ...
  useEffect(() => {
    setGrade(activeRow?.submission?.grade != null ? String(activeRow.submission.grade) : '');
    setFeedback(activeRow?.submission?.feedback || '');
  }, [activeRow?.submission?.feedback, activeRow?.submission?.grade, activeStudentId]);
                      ...
                      <tr key={row.member.member_id} className="border-b border-slate-100 last:border-b-0 transition-colors hover:bg-indigo-50/30">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-1.5">
                            {submission ? (
                              <>
                                <Button size="sm" className="h-8 rounded-lg bg-indigo-600 px-3 text-xs font-medium text-white shadow-sm hover:bg-indigo-700" onClick={() => setActiveStudentId(row.member.member_id)}>
                                ...
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent align="end" className="w-44 rounded-lg border-slate-200 p-1 shadow-lg">
                                    <DropdownMenuItem onClick={() => setActiveStudentId(row.member.member_id)} className="gap-2 rounded-md text-xs font-medium">
