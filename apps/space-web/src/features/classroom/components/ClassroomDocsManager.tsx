...
export function ClassroomDocsManager({
    ...
    if (
      !window.confirm(
        t(
          ...
          `Delete folder "${folder.name}" and all subfolders? Documents will be moved to root folder.`,
        ),
      )
    ) {
      return;
  ...
  const handleDeleteDoc = async (uid: string) => {
    if (!canManage) return;
    if (!window.confirm(t('classroom.docs.confirm_delete_doc', 'Delete this document?'))) return;
    try {
