...
import type { TeacherContact, StudentDetail } from '@lms/types';
...
export class StudentApiClient extends BaseRestApiClient {
  ...
  async getStudentSubmissions(classroomUid: string, consumerUid: string) {
    return this.get<{ exam: { uid: string; title: string; status: string; due_date: string | null }; submission: import('@/features/exam/types').ExamSubmission | null }[]>(
...
export { consumerApi } from './consumer';
