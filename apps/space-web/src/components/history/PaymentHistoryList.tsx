  ...
  return (
    <div className="space-y-3">
      {items.map((p) => {
        ...
        return (
          <Card key={p.uid} className="border-slate-200 hover:border-indigo-300 hover:shadow-md transition-all">
            <CardContent className="p-4 sm:p-5 flex items-center gap-4">
              ...
              <div className="text-right shrink-0">
                ...
                <p className="text-[10px] text-slate-500 mt-0.5">Consumer: {p.consumer_id?.slice(0, 8) || '—'}</p>
              </div>
            </CardContent>
          </Card>
        );
...
function EmptyState() {
  return (
    <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-12 text-center">
      ...
      <p className="mt-1 text-[12px] text-slate-500">Các giao dịch thanh toán từ học viên sẽ hiển thị tại đây.</p>
    </div>
  );
}
