  ...
  return (
    <section className="bg-white rounded-lg border border-slate-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-slate-900">{t('portfolio.me.featured')}</h2>
        {isOwner && (
          <button
        ...
        <div className="space-y-3">
          {items.map((item, idx) => {
            ...
            return (
              <article
                key={item.uid}
                className={`flex gap-4 p-2 -mx-2 rounded-lg hover:bg-slate-50 group ${!isLast ? 'border-b border-slate-100 pb-3' : ''}`}
              >
                {v.file_url ? (
                  <img
                    src={v.file_url}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {tag && <p className="text-xs text-slate-500 mt-0.5">{tag}</p>}
                  {v.description && (
