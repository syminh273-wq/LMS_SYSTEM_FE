        ...
        {isEditable && (
          <div className="flex gap-1.5 mb-3">
            <input
              ...
              className="flex-1 min-w-0 px-2.5 py-1.5 text-[12.5px] border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary-brand/30 focus:border-primary-brand"
              maxLength={40}
            />
            <Button
              type="button"
        ...
        {skills.length === 0 ? (
          ...
          <ul className="flex flex-wrap gap-1.5">
            {skills.map((s) => (
              <li
                key={s}
                className={`inline-flex items-center gap-1 pl-2 pr-1 py-1 text-[11.5px] font-semibold rounded-md bg-primary-brand-light text-primary-brand border border-primary-brand/20 ${isEditable ? '' : 'pr-2'}`}
              >
                {s}
                {isEditable && (
                  <Button
                    ...
                    disabled={saving}
                    variant="ghost"
                    className="w-4 h-4 flex items-center justify-center text-primary-brand hover:text-primary-foreground hover:bg-primary-brand"
                    title="Xoá"
                  >
                    <X size={10} />
                  </Button>
