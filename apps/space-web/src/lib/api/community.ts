...
class CommunityApiClient extends BaseRestApiClient {
  ...
  async updateMyProfile(data: Partial<WorkspaceProfile>): Promise<WorkspaceProfile> {
    return this.patch<WorkspaceProfile>('/api/v1/space/social/profile/me/', data);
  }
  async getPublicProfile(ownerId: string): Promise<WorkspaceProfile> {
    return this.get<WorkspaceProfile>(`/api/v1/space/social/profile/${ownerId}/`);
  }
  async uploadAvatar(file: File): Promise<{ url: string; avatar_url: string }> {
    const fd = new FormData();
    fd.append('file', file);
    return this.post('/api/v1/space/social/profile/me/avatar/', fd);
  }
  async uploadCover(file: File): Promise<{ url: string; cover_url: string }> {
    const fd = new FormData();
    fd.append('file', file);
    return this.post('/api/v1/space/social/profile/me/cover/', fd);
  }

  // ── Direct Conversations (chat is shared) ─────────────────────────────────
