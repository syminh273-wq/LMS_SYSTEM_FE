...
export function FaceClassroomGate({ classroomUid, children }: Props) {
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        // Check if the student already has a valid session (within 8 h)
        const session = await faceApi.classroomSessionStatus(classroomUid);
        if (session.is_verified) {
          setGateState('verified');
          return;
        }

        // Check enrollment before opening camera
    ...
    void checkSession();
    return () => stopCamera();
  }, [classroomUid, openCamera, stopCamera]);
  ...
  const handleVerify = async () => {
    ...
    try {
      const result = await faceApi.verifyForClassroom(classroomUid, image);
        ...
        <div className="w-full max-w-sm space-y-5 rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-8 text-center shadow-sm">
          ...
          <div>
            ...
            <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">
              Bạn cần đăng ký khuôn mặt trước khi vào lớp học.
            </p>
          </div>
          <Button
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/50 p-4">
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
        <div className="border-b border-slate-100 dark:border-slate-800 px-6 py-5 text-center">
          ...
          <h2 className="text-lg font-black text-slate-900 dark:text-slate-100">Xác minh để vào lớp</h2>
          <p className="mt-1 text-sm font-medium text-slate-500 dark:text-slate-400">
            Nhìn thẳng vào camera rồi nhấn xác minh — phiên có hiệu lực 8 giờ
          </p>
        </div>
            ...
            <div className="space-y-3 p-5">
              ...
              >
                {gateState === 'verifying' ? (
                  ...
                  <><CheckCircle2 size={16} className="mr-2" />Xác minh và vào lớp</>
                )}
              </Button>
            </div>
          </>
