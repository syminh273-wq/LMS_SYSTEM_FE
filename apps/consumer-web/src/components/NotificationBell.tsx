  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        ...
        aria-label={t('feed.notifications.aria_label')}
      >
      ...
      {open && (
        <div className="absolute right-0 top-11 z-50 w-80 rounded-xl border border-border bg-card shadow-xl">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            <span className="text-sm font-semibold text-foreground">{t('feed.notifications.title')}</span>
            {unreadCount > 0 && (
              <button
              ...
              >
                <CheckCheck size={13} />
                {t('feed.notifications.mark_all_read')}
              </button>
            )}
          </div>
...
function formatTime(
  ...
  if (diff < 60) return t('feed.notifications.just_now');
  if (diff < 3600) return t('feed.notifications.minutes_ago', undefined, { count: Math.floor(diff / 60) });
  if (diff < 86400) return t('feed.notifications.hours_ago', undefined, { count: Math.floor(diff / 3600) });
  const days = Math.floor(diff / 86400);
  if (days < 7) return t('feed.notifications.days_ago', undefined, { count: days });
