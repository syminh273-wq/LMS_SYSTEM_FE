...
const STATUS_META: Record<PaymentStatus, { label: string; icon: React.ElementType; cls: string }> = {
  ...
  CANCELLED: { label: 'Đã hủy', icon: AlertTriangle, cls: 'text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/50 ring-slate-200' },
};
...
export function InvoiceCard({ payment, classroomName, courseName, teacherName }: InvoiceCardProps) {
  ...
  return (
    <Card className="overflow-hidden border-slate-200 dark:border-slate-700 shadow-sm">
      <CardHeader className="bg-gradient-to-r from-indigo-50 via-white to-white border-b border-slate-100 dark:border-slate-800 px-6 py-5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-base font-bold text-slate-900 dark:text-slate-100">Hóa đơn thanh toán</CardTitle>
              <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-0.5">Mã đơn hàng #{payment.order_id.slice(0, 8).toUpperCase()}</p>
            </div>
          </div>
      ...
      <CardContent className="p-6 space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Mô tả">
            <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{payment.order_info || 'Thanh toán đơn hàng'}</p>
            {isClassroom && classroomName && (
              <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-0.5">Lớp: {classroomName}</p>
            )}
            {!isClassroom && courseName && (
              <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-0.5">Khóa học: {courseName}</p>
            )}
            {teacherName && isClassroom && (
              <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-0.5">Giáo viên: {teacherName}</p>
            )}
          </Field>

          <Field label="Số tiền">
            <p className="text-2xl font-extrabold text-indigo-600">{formatVND(payment.amount)}</p>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Đã bao gồm VAT (nếu có)</p>
          </Field>

          <Field label="Thời gian tạo">
            <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{formatDate(payment.created_at)}</p>
          </Field>

          {payment.updated_at && payment.updated_at !== payment.created_at && (
            <Field label="Cập nhật lần cuối">
              <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{formatDate(payment.updated_at)}</p>
            </Field>
          )}

          <Field label="Mã giao dịch MoMo" full>
            <div className="flex items-center gap-2">
              <code className="px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-800 text-[12px] font-mono text-slate-700 dark:text-slate-300 truncate">
              ...
              <button
                ...
                className="p-1.5 rounded-md hover:bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-slate-100 transition-colors"
...
function Field({ label, children, full }: { label: string; children: React.ReactNode; full?: boolean }) {
  return (
    <div className={cn('flex flex-col gap-1', full && 'sm:col-span-2')}>
      <span className="text-[11px] uppercase tracking-wide font-bold text-slate-500 dark:text-slate-400">{label}</span>
      {children}
    </div>
  );
}
