...
function FeatureCard({
  ...
  return (
    <div className="group snap-start shrink-0 w-64 rounded-xl border border-border overflow-hidden hover:border-primary/50 hover:shadow-md transition-all bg-card relative">
      {isOwner && (
        <DropdownMenu>
          <DropdownMenuTrigger className="absolute top-2 right-2 z-10 w-7 h-7 bg-background/90 backdrop-blur flex items-center justify-center text-muted-foreground opacity-0 group-hover:opacity-100">
      ...
      <div className="relative aspect-[16/9] bg-muted overflow-hidden">
        {feature.image ? (
          feature.fileType === 'application/pdf' ? (
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="block w-full h-full bg-gradient-to-br from-rose-50 to-amber-50 flex flex-col items-center justify-center text-destructive gap-1"
            >
        ...
        {feature.url && (
          <a
            ...
            className="absolute bottom-2 right-2 w-7 h-7 rounded-full bg-background/90 backdrop-blur hover:bg-background text-muted-foreground hover:text-primary flex items-center justify-center shadow-sm"
            title="Mở link"
          >
