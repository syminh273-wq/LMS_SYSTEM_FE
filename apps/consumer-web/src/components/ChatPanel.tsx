...
export function ChatPanel({
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-background">
      <div className="px-4 py-2 border-b border-border flex items-center justify-between text-xs text-muted-foreground bg-background/80 backdrop-blur">
          ...
          grouped.map((group, gi) => (
            <div key={gi} className="space-y-2">
              <div className="flex items-center justify-center">
                <span className="px-3 py-0.5 text-[10px] font-semibold text-muted-foreground bg-muted rounded-full uppercase tracking-wide">
                  {group.day}
                </span>
              </div>
              {group.items.map((m, i) => {
                      ...
                      <div
                        className={
                          me
                            ...
                            : `bg-card border border-border text-foreground shadow-sm ${bubbleRadius} px-3.5 py-2`
                        }
                      >
                        {m.attachment?.url && (
                          <a
