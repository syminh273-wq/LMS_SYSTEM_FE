    ...
    <WorkspaceShell>
      <div className="mx-auto w-full max-w-2xl py-6 sm:py-8">
        <div className="mb-4 flex items-center gap-3">
          <Button
            onClick={() => router.back()}
            className="h-9 w-9 rounded-full bg-card border border-border flex items-center justify-center text-muted-foreground hover:bg-muted transition-colors"
            aria-label="Quay lại"
          >
            <ArrowLeft size={16} />
          </Button>
        ...
        <div className="bg-card border border-border rounded-xl card-elevated overflow-hidden">
          <div className="p-3 border-b border-border">
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-10 pl-9 pr-9 rounded-lg bg-muted border border-border text-[13.5px] outline-none focus:bg-card focus:border-primary/30 focus:ring-2 focus:ring-primary/20 transition-colors"
              />
              {query && (
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => setQuery('')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 h-6 w-6 rounded-full text-muted-foreground hover:text-muted-foreground hover:bg-muted flex items-center justify-center"
                  aria-label="Xóa"
                >
                  <X size={12} />
                </Button>
          ...
          {loading ? (
            ...
            <div className="py-16 text-center">
              <p className="text-[13.5px] text-muted-foreground mb-3">{error}</p>
              <Button
                onClick={() => window.location.reload()}
                className="inline-flex items-center h-9 px-4 rounded-full bg-primary text-white text-[12.5px] font-semibold hover:bg-primary"
              >
                Thử lại
              </Button>
            </div>
            ...
            <div className="py-16 text-center px-6">
              ...
              <p className="font-semibold text-foreground text-[15px]">Bạn chưa theo dõi ai</p>
              <p className="text-[13px] text-muted-foreground mt-1 mb-4">
                Khám phá những người bạn có thể quan tâm.
              </p>
              <Link
                href="/consumer/feed"
                className="inline-flex items-center h-10 px-5 rounded-full bg-primary text-white text-[13px] font-semibold hover:bg-primary"
              >
                Khám phá gợi ý
              </Link>
            </div>
                  ...
                  >
                    ...
                    >
                      <Avatar className="h-11 w-11 ring-2 ring-white shadow-sm shrink-0">
                        ...
                        <AvatarFallback className="bg-gradient-to-br from-primary to-primary text-[12px] font-black text-white">
                          {initials}
                        </AvatarFallback>
                      </Avatar>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <Button
                        ...
                        className="inline-flex items-center gap-1.5 h-9 px-3 rounded-full border border-border text-[12.5px] font-semibold text-foreground hover:bg-muted hover:text-primary transition-colors"
                        title="Nhắn tin"
                      >
                        <Mail size={13} />
