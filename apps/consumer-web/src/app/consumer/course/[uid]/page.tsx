...
export default function CourseDetailPage({ params }: { params: Promise<{ uid: string }> }) {
      ...
      <div className="max-w-2xl mx-auto py-12 space-y-4">
        ...
        <Card>
          <CardContent className="text-center space-y-4">
            <Lock size={48} className="mx-auto text-warning" />
      ...
      <div className="max-w-2xl mx-auto py-12">
        <Card>
          <CardContent className="text-center space-y-3">
            ...
            <p className="text-muted-foreground">{t('course.preview.not_found', 'Course not found')}</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <Button variant="ghost" onClick={() => router.push('/consumer/course')} className="gap-2">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-border">
                {lessons.map((l, idx) => (
                  <div
                    key={l.uid}
                    className={`p-4 flex items-center gap-3 cursor-pointer hover:bg-muted/30 ${activeVideo === l.video_url ? 'bg-primary/10' : ''}`}
                  ...
                  >
                    ...
                    {l.video_url && <Play size={18} className="text-primary flex-shrink-0" />}
