...
export function FeedLeftSidebar({ profile, followingCount = 0 }: { profile: Profile; followingCount?: number }) {
  ...
  return (
    <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
        ...
        <div className="flex flex-col items-center text-center">
          <div className="relative">
            <Avatar className="h-20 w-20 ring-4 ring-white shadow-sm">
              ...
              <AvatarFallback className="bg-gradient-to-br from-primary to-primary text-lg font-black text-white">
                {initials}
              </AvatarFallback>
            </Avatar>
            <span className="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-success rounded-full border-2 border-white" />
          </div>
          <h2 className="mt-3 text-[15px] font-bold text-foreground truncate max-w-full">
            {profile.full_name}
          </h2>
          {profile.email && (
            <p className="text-[12px] text-muted-foreground truncate max-w-full mt-0.5">
              {profile.email}
            </p>
          )}
        </div>
      ...
      <nav className="bg-card border border-border rounded-xl p-2 card-elevated">
            ...
            return (
              <li key={path}>
                <Button
                  ...
                  className={cn(
                    'w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-[13.5px] font-medium text-muted-foreground',
                ...
                >
                  <span
                    data-active={active || undefined}
                    className={cn(
                      'w-8 h-8 rounded-lg flex items-center justify-center shrink-0 bg-muted text-muted-foreground',
                      'data-[active=true]:text-foreground data-[active=true]:font-semibold',
                    )}
                  >
                    <Icon size={15} />
                  ...
                  {typeof badge === 'number' && badge > 0 && (
                    <span
                      data-active={active || undefined}
                      className={cn(
                        'shrink-0 text-[11px] font-bold px-1.5 py-0.5 rounded-md min-w-[22px] text-center bg-muted text-muted-foreground',
                        'data-[active=true]:text-foreground data-[active=true]:font-semibold',
                      )}
                    >
                      {badge > 999 ? '999+' : badge}
      ...
      <Button
        ...
        className="w-full inline-flex items-center justify-center gap-2 h-11 bg-card border border-border rounded-xl text-[13.5px] font-semibold text-foreground hover:bg-muted hover:text-primary transition-colors card-elevated"
      >
        <PenLine size={14} />
        Chỉnh sửa hồ sơ
      </Button>
