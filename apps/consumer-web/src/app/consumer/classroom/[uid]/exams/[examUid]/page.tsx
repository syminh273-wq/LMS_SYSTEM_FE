...
export default function ConsumerExamDetailPage({ params }: { params: Promise<{ uid: string; examUid: string }> }) {
  ...
  if (error || !exam) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/50 p-4">
        <div className="w-full max-w-md space-y-4 rounded-3xl border border-rose-100 bg-white dark:bg-slate-900 p-8 text-center shadow-sm">
          <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-rose-100 text-rose-600">
            <AlertCircle size={32} />
          </div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Không thể tải bài kiểm tra</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">{error || 'Không tìm thấy bài kiểm tra'}</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900/50 pb-20 text-slate-900 dark:text-slate-100 lg:pb-0">
          ...
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-6">
            <div className="flex min-w-0 items-center gap-3 sm:gap-4">
              <Button
                ...
                className="shrink-0 rounded-xl hover:bg-slate-100 dark:bg-slate-800"
              >
                <ArrowLeft size={20} />
              </Button>
              <div className="min-w-0">
                ...
                <h1 className="truncate text-base font-black text-slate-900 dark:text-slate-100 sm:text-lg">{exam.title}</h1>
              </div>
            </div>
            <div className="ml-12 flex flex-wrap items-center gap-2 text-xs font-bold sm:ml-auto">
              ...
              {exam.due_date && (
                <span className="inline-flex items-center gap-1.5 rounded-lg bg-slate-50 dark:bg-slate-900/50 px-3 py-2 text-slate-500 dark:text-slate-400">
                  <Calendar size={13} /> {formatDateTime(exam.due_date)}
                </span>
              )}
              <span className="rounded-lg bg-slate-50 dark:bg-slate-900/50 px-3 py-2 text-slate-500 dark:text-slate-400">Tối đa: <strong className="text-slate-800 dark:text-slate-200">{maxGrade}</strong></span>
            </div>
          </div>
        </header>
          ...
          <div className="min-w-0 space-y-5">
            ...
            {exam.exam_type === 'quiz' && (
              <div className="space-y-6">
                {submission ? (
                  <>
                    <section className="overflow-hidden rounded-2xl border border-emerald-100 bg-white dark:bg-slate-900 shadow-sm shadow-emerald-100/40">
                      <div className="flex items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 bg-emerald-50 px-5 py-6">
                        <div>
                      ...
                      {submission.quiz_result && (
                        <div className="px-5 pt-4">
                          <div className="mb-1 flex items-center justify-between text-xs font-bold text-slate-500 dark:text-slate-400">
                            <span>Số câu đúng: <strong className="text-slate-800 dark:text-slate-200">{submission.quiz_result.correct_count}/{submission.quiz_result.total}</strong></span>
                      ...
                      <div className="grid grid-cols-1 divide-y divide-slate-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0 mt-2">
                        <div className="p-5">
                          ...
                          <p className="text-sm font-bold text-slate-700 dark:text-slate-300">Nộp lúc: {formatDateTime(submission.submitted_at)}</p>
                          <p className="text-xs font-medium text-slate-500 dark:text-slate-400">{isLate ? 'Nộp trễ hạn' : 'Nộp đúng hạn'}</p>
                        </div>
                        <div className="p-5">
                          ...
                          {submission.feedback && (
                            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">{submission.feedback}</p>
                          )}
                        </div>
                      </div>
                    </section>
                      ...
                      <div className="space-y-3">
                        ...
                        {submission.quiz_result.results.map((item, idx) => (
                          <div
                            ...
                            className={`overflow-hidden rounded-2xl border bg-white dark:bg-slate-900 shadow-sm ${item.is_correct ? 'border-emerald-100' : 'border-rose-100'}`}
                          >
                            ...
                            <div className="flex flex-wrap gap-2 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 px-4 py-3">
                              {item.chosen && (
                            ...
                            {item.explanation && (
                              <div className="border-t border-amber-100 bg-amber-50 px-4 py-2 text-xs font-medium leading-5 text-slate-600 dark:text-slate-400">
                  ...
                  <div className="flex h-64 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
                    <Loader2 className="mb-4 animate-spin text-indigo-500" size={32} />
                    <p className="text-sm font-bold text-slate-500 dark:text-slate-400">Đang tải câu hỏi...</p>
                  </div>
                ) : (
                  <div className="space-y-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                      {quizData.questions.map((q: any, index: number) => (
                        <Card key={q.uid} className="overflow-hidden rounded-2xl border-slate-200 dark:border-slate-700 shadow-sm transition-all hover:border-indigo-200">
                          <CardHeader className="bg-slate-50 dark:bg-slate-900/50/50 pb-3 pt-4">
                            <div className="flex items-center gap-2">
                              ...
                              <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 leading-relaxed">{q.question_text}</h3>
                            </div>
                          </CardHeader>
                              ...
                              {['a', 'b', 'c', 'd'].map((letter) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`flex items-center gap-3 rounded-xl border-2 p-3 text-left transition-all ${isSelected ? 'border-indigo-500 bg-indigo-50 shadow-sm' : 'border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:bg-slate-900/50/50'}`}
                                  >
                                    <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-black uppercase ${isSelected ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>
                                      {letter}
                                    </span>
                                    <span className={`text-sm font-medium ${isSelected ? 'text-indigo-900' : 'text-slate-700 dark:text-slate-300'}`}>{optionText}</span>
                      ...
                      <div className="mt-8 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 text-center">
                        <p className="mb-4 text-sm font-medium text-slate-500 dark:text-slate-400">Kiểm tra kỹ các đáp án trước khi nộp bài. Bạn đã làm {Object.keys(quizAnswers).length}/{quizData.total_questions} câu.</p>
            ...
            {exam.exam_type === 'assignment' && (
              <>
                <section className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
                  <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 px-5 py-4 text-slate-800 dark:text-slate-200">
                    <FileText size={18} className="text-indigo-500" />
                    <h2 className="text-base font-black">Đề bài tự luận</h2>
                  </div>
                  <div className="p-4 sm:p-5">
                    {exam.description && (
                      <p className="mb-4 text-sm font-medium leading-6 text-slate-600 dark:text-slate-400">{exam.description}</p>
                    )}
                    {exam.content_type === 'markdown' ? (
                      ...
                      <div className="flex items-center justify-between rounded-xl bg-slate-50 dark:bg-slate-900/50 p-4">
                        <div className="flex items-center gap-3">
                          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-white dark:bg-slate-900 text-indigo-600 shadow-sm">
                            <FileText size={20} />
                          </div>
                          <div>
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{exam.meta?.name || 'Đề bài đính kèm'}</p>
                ...
                <section className="overflow-hidden rounded-2xl border border-indigo-100 bg-white dark:bg-slate-900 shadow-sm shadow-indigo-100/40">
                  <form id="exam-submission-form" onSubmit={handleSubmit}>
                    <div className="flex items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 px-5 py-4">
                      <div>
                        ...
                        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Upload file bài làm (PDF, DOCX, ZIP, JPG, PNG)</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 p-5">
                      <label
                        ...
                        className={`group flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed p-5 text-center transition-all duration-200 ${
                          submissionDisabled
                            ? 'cursor-not-allowed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 text-slate-400'
                            : isDraggingFile
                      ...
                      >
                        <span className={`mb-3 flex h-11 w-11 items-center justify-center rounded-xl transition-transform ${submissionDisabled ? 'bg-slate-100 dark:bg-slate-800 text-slate-300' : 'bg-white dark:bg-slate-900 text-indigo-600 shadow-sm group-hover:scale-105'}`}>
                          <UploadCloud size={23} />
                        </span>
                        <span className="text-sm font-black text-slate-800 dark:text-slate-200">{answerFile?.name || (submission?.resource_url ? 'Kéo thả file mới để thay đổi bài làm' : 'Kéo thả file bài làm vào đây')}</span>
                        <span className="mt-1 text-xs font-medium text-slate-500 dark:text-slate-400">{submissionDisabled ? 'Đã hết hạn nộp bài' : 'hoặc bấm để chọn file từ máy tính'}</span>
                      ...
                      {(selectedPreviewFile || submittedFile) ? (
                        <div className="rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-3">
                          <div className="flex items-center gap-3">
                            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-white dark:bg-slate-900 text-indigo-600">
                              <FileText size={18} />
                            </span>
                            <div className="min-w-0 flex-1">
                              <p className="truncate text-sm font-bold text-slate-700 dark:text-slate-300">{(selectedPreviewFile || submittedFile)?.name}</p>
                              {submission?.submitted_at && !selectedPreviewFile && <p className="text-xs text-slate-500 dark:text-slate-400">Đã nộp lúc {formatDateTime(submission.submitted_at)}</p>}
                            </div>
                    ...
                    <div className="hidden border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 px-5 py-4 sm:block">
          ...
          <aside className="space-y-4 lg:sticky lg:top-5">
              ...
              <div className="bg-indigo-600 p-5 text-white">
                ...
                {grade !== null && (
                  <div className="mt-4 h-2 overflow-hidden rounded-full bg-indigo-400/40">
                    <div className="h-full rounded-full bg-white dark:bg-slate-900 transition-all duration-700" style={{ width: `${gradePercent}%` }} />
                  </div>
                )}
            ...
            <section className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h2 className="text-sm font-black text-slate-800 dark:text-slate-200">Nhận xét của giáo viên</h2>
              </div>
              {hasFeedback ? (
                <div className="rounded-xl bg-slate-50 dark:bg-slate-900/50 p-3.5">
                  <p className="whitespace-pre-wrap text-sm font-medium leading-6 text-slate-700 dark:text-slate-300">{submission?.feedback}</p>
                ...
                <div className="flex items-start gap-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/50 p-3 text-xs font-medium leading-5 text-slate-500 dark:text-slate-400">
        ...
        {exam.exam_type === 'assignment' && (
          <div className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-3 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] sm:hidden">
...
function SidebarRow({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3 py-3.5 text-xs">
      ...
      <span className="text-right font-bold text-slate-800 dark:text-slate-200">{value}</span>
    </div>
  );
}
...
function CompactProgress({ submission, isGraded, hasFeedback }: { submission: ExamSubmission | null; isGraded: boolean; hasFeedback: boolean }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm">
      <h2 className="mb-4 text-sm font-black text-slate-900 dark:text-slate-100">Tiến độ bài thi</h2>
        ...
        {steps.map((step, index) => (
          <div key={step.label} className="relative flex min-w-0 flex-1 flex-col items-center text-center">
            ...
            <span className={`relative z-[1] flex h-6 w-6 items-center justify-center rounded-full ${
              step.done ? 'bg-emerald-100 text-emerald-600' : step.current ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'
            }`}>
              {step.done ? <CheckCircle2 size={14} /> : <span className="h-2 w-2 rounded-full bg-current" />}
            </span>
            <div className="relative z-[1] mt-2 bg-white dark:bg-slate-900 px-1">
              <p className={`text-[10px] font-bold ${step.done || step.current ? 'text-slate-700 dark:text-slate-300' : 'text-slate-400'}`}>{step.label}</p>
            </div>
          </div>
        ))}
      </div>
...
function ExamResultSkeleton() {
  return (
    <div className="min-h-screen animate-pulse bg-slate-50 dark:bg-slate-900/50">
      <div className="h-16 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900" />
      <div className="mx-auto grid max-w-7xl gap-5 p-4 sm:p-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-5">
          <div className="h-80 rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
            <div className="h-full rounded-xl bg-slate-100 dark:bg-slate-800" />
          </div>
          <div className="h-32 rounded-2xl bg-white dark:bg-slate-900 p-5">
            <div className="h-full rounded-xl bg-slate-100 dark:bg-slate-800" />
          </div>
        </div>
        <div className="h-72 rounded-2xl bg-white dark:bg-slate-900 p-5">
          <div className="h-full rounded-xl bg-slate-100 dark:bg-slate-800" />
        </div>
      </div>
    </div>
...
function FilePreviewModal({ file, onClose }: { file: PreviewFile; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-2 sm:p-4">
      <div className="flex h-[85vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white dark:bg-slate-900 shadow-2xl">
        <div className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 px-4">
          <div className="min-w-0">
            <div className="truncate text-sm font-black text-slate-900 dark:text-slate-100">{file.name}</div>
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-slate-50 dark:bg-slate-900/50">
          {isImage ? (
...
function getResultStatusMeta(submission: ExamSubmission | null) {
  if (!submission) return { label: 'Chưa nộp', className: 'bg-slate-50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700' };
