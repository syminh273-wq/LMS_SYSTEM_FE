...
export class FaceApiClient extends BaseRestApiClient {
  ...
  async verifyGeneral(image: string): Promise<FaceVerifyResponse> {
    return this.post<FaceVerifyResponse>('/api/v1/consumer/face/verify/', { image });
  }
  ...
  async verifyClassroom(classroomUid: string, image: string): Promise<FaceClassroomVerifyResponse> {
    return this.verifyForClassroom(classroomUid, image);
  }

  async getEnrollStatus(): Promise<FaceEnrollStatusResponse> {
